...
class NeuronaLinealSinBIAS(object):
    """
    Parameters
    ------------
    alpha : float
    ...
    def fit(self, X, y):
        ...
        # Inicializar solo los pesos, sin el bias
        self.w_ = rgen.uniform(-0.5, 0.5, size=X.shape[1]) 
        # self.b_ = rgen.uniform(-0.5, 0.5)  # Eliminado el bias
        self.errors_ = []
        ph = 0  # manejador de la recta mientras se dibuja
        ErrorAnt = 0
        ErrorAct = 1
        ...
        while ((i < self.n_iter) and (np.abs(ErrorAnt - ErrorAct) > self.cotaE)):
            ...
            for xi, target in zip(X, y):
                ...
                update = self.alpha * errorXi
                self.w_ += update * xi
                # self.b_ += update  # Eliminado el bias
            ...
            # graficar la recta
            if (self.draw):
                ph = dibuPtosRecta(puntos, T, np.array([self.w_, -1], dtype=object), 0, self.title, ph)  # bias=0
    ...
    def net_input(self, X):
        """Calculate net input"""
        return np.dot(X, self.w_)  # Eliminado el bias
